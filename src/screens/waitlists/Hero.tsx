import bg from "@/assets/images/waitlist.png";
import Aurora from "@/components/ui/reactbits/Aurora";

import type { ReactNode } from "react";

export const Hero = ({
  title = "Kill the queue. Keep the vibe.",
  subtitle = "Self-check-in and access control that actually scales. NFC, QR, or WhatsApp.",
  background = bg,
  showAurora = true,
  children,
}: {
  title?: string;
  subtitle?: string;
  background?: string;
  showAurora?: boolean;
  children?: ReactNode;
}) => {
  return (
    <section
      style={{
        backgroundImage: `url(${background})`,
        backgroundSize: "cover",
        backgroundPosition: "center",
      }}
      className="py-10 md:py-15 relative h-screen w-full overflow-hidden"
    >
      {/* Aurora background */}
      {showAurora && (
        <div className="hidden md:block absolute inset-0 z-0">
          <Aurora
            colorStops={["#D2180A", "#EAEF9E", "#7725FC"]}
            blend={0.5}
            amplitude={1.0}
            speed={0.5}
          />
        </div>
      )}

      {/* Content */}
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-white gap-10 px-4">
        <div className="bubble">
          <span className="bubbleText">Rootscard is launching soon</span>
        </div>

        <div className="flex flex-col items-center justify-center text-center gap-5 max-w-[80%]">
          <h1 className="hOne titleFont font-bold relative">{title}</h1>
          <p className="hFour max-w-[80%] md:w-lg leading-8 mx-auto">
            {subtitle}
          </p>
        </div>

        {/* Slot for form or any other content */}
        {children}
      </div>
    </section>
  );
};
